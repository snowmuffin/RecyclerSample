...
class TableDetailViewModel(private val datasource: DataSource) : ViewModel() {

    /* Queries datasource to returns a table that corresponds to an id. */
    fun getTableForId(id: Long) : Table? {
        return datasource.getTableForId(id)
    }

    /* Queries datasource to remove a table. */
    fun removeTable(table: Table) {
        datasource.removeTable(table)
    }
}

class TableDetailViewModelFactory(private val context: Context) : ViewModelProvider.Factory {
